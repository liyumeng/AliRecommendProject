...
class BinReader(object):
    ...
    def readline(self):
        userid = struct.unpack('f',self._file.read(4))[0]
        itemid = struct.unpack('f',self._file.read(4))[0]
        label = struct.unpack('f',self._file.read(4))[0]
    ...
    def readData(filename):
        ...
        for i in xrange(reader.LineCount):
            userid = struct.unpack('f',reader._file.read(4))[0]
            itemid = struct.unpack('f',reader._file.read(4))[0]
            label = struct.unpack('f',reader._file.read(4))[0]
            posiCount+=label
            x = array.array('f')
